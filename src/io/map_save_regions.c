...
int map_save_read_natural_regions(FILE *file, int save_version) {
    ...
    if (save_version >= 5) {
        LegacyNaturalRegionV6 legacy[MAX_NATURAL_REGIONS];
        int i;
        if (!read_block(file, legacy, sizeof(LegacyNaturalRegionV6), (size_t)region_count)) return 0;
