...
static void draw_header(HDC hdc, UiCursor *cursor, int civ_id, int other_id, DiplomacyRelation relation) {
    ...
    RECT tag_rect = {row.right - 82, row.top + 2, row.right, row.bottom - 2};
    char title[160];
    ...
    fill_rect(hdc, tag_rect, status_color_for_pair(civ_id, other_id, relation));
    draw_center_text(hdc, tag_rect, status_label_for_pair(civ_id, other_id, relation), RGB(246, 248, 250));
}
...
static void bar_row(HDC hdc, UiCursor *cursor, const char *label, const char *value,
    ...
    RECT label_rect = {row.left, row.top, row.left + 100, row.bottom};
    RECT value_rect = {row.left + 100, row.top, row.left + 152, row.bottom};
    RECT bar = {row.left + 156, row.top + 7, row.right, row.bottom - 7};
...
static void metric_chip(HDC hdc, RECT rect, IconId icon, const char *label, const char *value, COLORREF accent) {
    ...
    RECT label_rect = {rect.left + 27, rect.top, rect.left + (rect.right - rect.left) / 2 + 10, rect.bottom};
...
static const char *easing_label(DiplomacyRelation relation) {
    if (relation.state == DIPLOMACY_TRUCE) return tr("Truce cooling", "停战冷却");
    if (relation.easing_years > 0) return tr("Easing", "缓和中");
    if (relation.trade_fit >= 45 && relation.border_tension < 45 && relation.resource_conflict < 45) return tr("Easing", "缓和中");
    if (relation.border_tension >= 55 || relation.resource_conflict >= 55) return tr("Worsening", "仍在恶化");
    return tr("Stable", "稳定");
}

static void draw_peace_tense(HDC hdc, UiCursor *cursor, int civ_id, int other_id, DiplomacyRelation relation) {
    char num[48], border[32], known[32], gap[64];
    ...
    snprintf(known, sizeof(known), "%d", relation.years_known);
    chip_row3(hdc, cursor, ICON_COMMERCE, tr("Trade", "贸易"), level_label(relation.trade_fit),
              ICON_TERRITORY, tr("Border", "边界"), border,
              ICON_GEOGRAPHY, tr("Barrier", "屏障"), level_label(relation.natural_barrier));
    snprintf(gap, sizeof(gap), "%s %d / %s %d",
             tr("Tension", "紧张"), max(0, relation.border_tension - 34),
             tr("Relation", "关系"), max(0, 46 - relation.relation_score));
    chip_row3(hdc, cursor, ICON_COHESION, tr("Peace rule", "回和平条件"), tr("T<35 R>45", "紧张<35 关系>45"),
              ICON_DISORDER, tr("Current gap", "当前缺口"), gap,
              ICON_HARBOR, tr("Contact", "接触"), ui_language ? war_front_reason_zh(flags) : war_front_reason_en(flags));
    chip_row3(hdc, cursor, ICON_CITY_STAGE, tr("Known", "已知"), known,
              ICON_ATTACK, tr("Conflict", "冲突"), level_label(relation.resource_conflict),
              ICON_GOVERNANCE, tr("Trend", "状态"), easing_label(relation));
}
...
static void draw_war_truce(HDC hdc, UiCursor *cursor, int civ_id, int other_id, DiplomacyRelation relation) {
    ...
    char a[48], b[48], c[48], d[48], span[48];
    int flags = war_front_flags(civ_id, other_id);
    if (war.active) {
        ...
        ui_format_months(d, sizeof(d), max(0, 3 - (war.years % 3)) * 12, UI_MONTH_ZERO_NOW);
        chip_row3(hdc, cursor, ICON_BATTLE, tr("Losses", "战损"), a,
                  ICON_ATTACK, tr("Enemy", "敌方"), b,
                  ICON_COUNTRY_DEFENSE, tr("Wins", "胜负"), c);
        snprintf(a, sizeof(a), "%d/%d", peace_pressure_for_side(civ_id, war), peace_pressure_for_side(other_id, war));
        chip_row3(hdc, cursor, ICON_MILITARY, tr("Next battle", "下次战斗"), d,
                  ICON_COHESION, tr("Peace pressure", "议和倾向"), a,
        ...
        snprintf(a, sizeof(a), "%d", relation.border_tension);
...
static void format_tribute(VassalTributeBreakdown br, int sign, char *out, size_t size) {
    char prefix = sign < 0 ? '-' : '+';
    snprintf(out, size, tr("F%c%d W%c%d Wood%c%d Stone%c%d Ore%c%d Herd%c%d Total%c%d",
                           "粮%c%d 水%c%d 木%c%d 石%c%d 矿%c%d 畜%c%d 总%c%d"),
             prefix, br.food, prefix, br.water, prefix, br.wood, prefix, br.stone,
             prefix, br.minerals, prefix, br.livestock, prefix, br.total);
}

static void draw_vassal_card(HDC hdc, UiCursor *cursor, int civ_id, int other_id) {
    char a[160], b[48], c[48];
    if (vassal_is_direct(civ_id, other_id)) {
        VassalTributeBreakdown br = vassal_resource_tribute_breakdown_from(other_id);
        bar_row(hdc, cursor, tr("Resource flow", "资源流向"), "40%", 40, RGB(154, 105, 178));
        bar_row(hdc, cursor, tr("Army call-up", "军队调用"), "70%", 70, RGB(188, 88, 154));
        format_tribute(br, 1, a, sizeof(a));
        snprintf(b, sizeof(b), "%d / %d", vassal_callable_soldiers(other_id), vassal_total_soldiers(other_id));
        snprintf(c, sizeof(c), "%d", vassal_support_used_by_overlord(civ_id, other_id));
        chip_row3(hdc, cursor, ICON_FOOD, tr("Resource tribute", "资源贡献"), a,
                  ICON_MILITARY, tr("Callable army", "可调用军队"), b,
                  ICON_ATTACK, tr("Front use", "当前战线使用"), c);
        snprintf(b, sizeof(b), "%d", vassal_garrison_soldiers(other_id));
        snprintf(c, sizeof(c), "+%d", vassal_governance_disorder(civ_id));
        snprintf(a, sizeof(a), "%d", vassal_support_casualties(other_id));
        chip_row3(hdc, cursor, ICON_COUNTRY_DEFENSE, tr("Vassal stay", "附庸留守"), b,
                  ICON_DISORDER, tr("Support losses", "支援伤亡"), a,
                  ICON_GOVERNANCE, tr("Governance", "治理负担"), c);
        ...
        bar_row(hdc, cursor, tr("Army callable", "军队可调用"), "70%", 70, RGB(188, 88, 154));
        format_tribute(br, -1, a, sizeof(a));
        ...
        chip_row3(hdc, cursor, ICON_FOOD, tr("Extracted", "资源被抽取"), a,
                  ICON_MILITARY, tr("Callable", "被宗主可调用"), b,
                  ICON_COUNTRY_DEFENSE, tr("Home guard", "本土留守"), c);
        snprintf(c, sizeof(c), "%d", vassal_support_casualties(civ_id));
        chip_row3(hdc, cursor, ICON_GOVERNANCE, tr("Autonomy", "外交自主"), tr("None", "无"),
                  ICON_HARBOR, tr("Protection", "保护状态"), war_active_for_civ(other_id) ? tr("At war", "战争中") : tr("Peace", "和平"),
                  ICON_DISORDER, tr("Support losses", "支援伤亡"), c);
    } else {
        ...
        chip_row3(hdc, cursor, ICON_GOVERNANCE, tr("Autonomy", "自主权"), tr("None", "无"),
                  ICON_COUNTRY_DEFENSE, tr("Represented by", "外交代表"), a,
...
void draw_diplomacy_relation_card(HDC hdc, UiCursor *cursor, int civ_id,
    ...
    RECT card = {cursor->x, cursor->y, cursor->x + cursor->width, cursor->y + 194};
