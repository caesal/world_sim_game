...
static const char *slider_help(int index) {
    switch (index) {
        ...
        case UI_SLIDER_REGION_SIZE:
            return tr("Natural region size before civilizations. Changes regenerate regions.", "文明出现前的自然区域大小；改动会重生成自然区域。");
        default:
            return "";
    }
}
...
static void draw_worldgen_slider(HDC hdc, const WorldgenLayout *layout, int index,
    ...
    draw_text_rect(hdc, slider->help, help, ui_theme_color(UI_COLOR_TEXT_DIM),
                   DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
...
static const char *metric_help(int index) {
    switch (index) {
        case WORLDGEN_METRIC_MILITARY:
            return tr("Army organization, weapons, war readiness.", "军队组织、武器和战争准备。");
        case WORLDGEN_METRIC_LOGISTICS:
            return tr("Supply, movement, migration, distant expansion.", "补给、移动、迁徙和远距离扩张。");
        case WORLDGEN_METRIC_GOVERNANCE:
            return tr("Administration, order, taxes, controlled expansion.", "行政、秩序、税收和受控扩张。");
        case WORLDGEN_METRIC_COHESION:
            return tr("Social stability, unity, assimilation, anti-collapse.", "社会稳定、统一、同化和抗崩溃。");
        case WORLDGEN_METRIC_PRODUCTION:
            return tr("Construction, tools, infrastructure, resource development.", "建设、工具、基础设施和资源开发。");
        case WORLDGEN_METRIC_COMMERCE:
            return tr("Money, markets, ports, trade exchange.", "金钱、市场、港口和贸易交换。");
        case WORLDGEN_METRIC_INNOVATION:
            return tr("Learning, tools, systems, long-term growth.", "学习、工具、制度和长期成长。");
        default:
            return "";
    }
}
...
static void draw_civ_color_palette(HDC hdc, const WorldgenLayout *layout) {
    ...
    for (i = 0; i < CIV_COLOR_PALETTE_COUNT; i++) {
        ...
        if (i == selected_civ_color_index) {
            RECT highlight = swatch;
            HBRUSH brush = CreateSolidBrush(RGB(255, 250, 210));
            InflateRect(&highlight, -1, -1);
...
static void draw_civilization_placement(HDC hdc, const WorldgenLayout *layout, const char **active_tooltip) {
    ...
    for (i = 0; i < WORLDGEN_CORE_METRIC_COUNT; i++) {
        draw_text_rect(hdc, layout->metric_label[i],
                       ...
                       ui_theme_color(UI_COLOR_TEXT_MUTED), DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
        draw_text_rect(hdc, layout->metric_help[i], metric_help(i), ui_theme_color(UI_COLOR_TEXT_DIM),
...
void draw_worldgen_panel(HDC hdc, RECT client, int x, HFONT title_font, HFONT body_font) {
    ...
    draw_text_rect(hdc, layout.initial_help,
                   ...
                   ui_theme_color(UI_COLOR_TEXT_DIM), DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
    draw_section_rect(hdc, layout.physical_section, tr("Physical World", "物理世界"));
    ...
    draw_section_rect(hdc, layout.terrain_section, tr("Advanced Terrain Bias", "高级地形偏好"));
