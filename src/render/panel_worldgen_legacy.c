#include "panel_worldgen_legacy.h"
...
static UiClayState legacy_control_state(RECT viewport, RECT rect,
                                        int selected) {
    int hover = ui_worldgen_panel_hit_test(
        viewport, rect, hover_x, hover_y);
    int pressed = hover && (GetKeyState(VK_LBUTTON) & 0x8000);
    return ui_clay_state_from_flags(hover, pressed, selected, 0);
}
...
static void draw_input_frame(HDC hdc, const WorldgenLayout *layout, RECT rect) {
    ...
    state = legacy_control_state(layout->viewport, rect, 0);
...
static void draw_random_button(HDC hdc, RECT viewport, RECT rect) {
    RECT icon = {rect.left + 7, rect.top + 5, rect.left + 20, rect.top + 18};
    RECT text = {rect.left + 22, rect.top, rect.right - 6, rect.bottom};
    UiClayState state = legacy_control_state(viewport, rect, 0);
...
static void draw_section_rect_with_button(HDC hdc, RECT viewport, RECT rect,
                                          RECT button, const char *title) {
    ...
    draw_random_button(hdc, viewport, button);
}

static void maybe_tooltip(RECT viewport, RECT rect, const char *text,
                          const char **active_tooltip) {
    if (ui_worldgen_panel_hit_test(viewport, rect, hover_x, hover_y))
        *active_tooltip = text;
}
...
static void draw_map_size_selector(HDC hdc, const WorldgenLayout *layout) {
    ...
    for (int i = 0; i < MAP_SIZE_COUNT; i++) {
        ...
        state = legacy_control_state(layout->viewport, button,
                                     i == pending_map_size);
...
static const char *slider_help(int index) {
    switch (index) {
        ...
        case UI_SLIDER_REGION_SIZE:
            return tr("Natural region size before civilizations. Applies on the next F5.", "文明出现前的自然区域大小；下次 F5 时生效。");
        default:
            return "";
    }
}
...
static void draw_worldgen_slider(HDC hdc, const WorldgenLayout *layout, int index,
    ...
    state = legacy_control_state(layout->viewport, slider->hit, 0);
    ...
    draw_text_rect_clipped(hdc, slider->help, help, ui_theme_color(UI_COLOR_TEXT_DIM),
                           DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
    maybe_tooltip(layout->viewport, slider->hit, help, active_tooltip);
}
...
void draw_civ_color_palette(HDC hdc, const WorldgenLayout *layout) {
    ...
    for (int i = 0; i < CIV_COLOR_PALETTE_COUNT; i++) {
        ...
        state = legacy_control_state(layout->viewport, swatch,
                                     i == selected_civ_color_index);
...
static void draw_civ_color_picker_preview(HDC hdc, const WorldgenLayout *layout) {
    ...
    state = legacy_control_state(layout->viewport, preview, 0);
...
static void draw_metric_inputs(HDC hdc, const WorldgenLayout *layout, const char **active_tooltip) {
    ...
    for (int i = 0; i < WORLDGEN_CORE_METRIC_COUNT; i++) {
        ...
        maybe_tooltip(layout->viewport, layout->metric_label[i],
                      metric_help(i), active_tooltip);
        maybe_tooltip(layout->viewport, layout->metric_help[i],
                      metric_help(i), active_tooltip);
    }
}

static void draw_civilization_placement(HDC hdc, const WorldgenLayout *layout, const char **active_tooltip) {
    draw_section_rect_with_button(hdc, layout->viewport, layout->civ_section,
                                  layout->civ_random_button,
...
void draw_worldgen_legacy_panel(HDC hdc,
    ...
    if (!legacy) return;
    ui_worldgen_view_build_legacy_layout(legacy, &layout);
    ...
    draw_text_rect(hdc, layout.initial_help,
                   ...
                   ui_theme_color(UI_COLOR_TEXT_DIM), DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
    draw_section_rect_with_button(hdc, layout.viewport, layout.physical_section,
                                  layout.physical_random_button,
    ...
    draw_section_rect_with_button(hdc, layout.viewport, layout.terrain_section,
                                  layout.terrain_random_button,
