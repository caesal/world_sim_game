...
static DiplomacyDisplayGroup display_group_for_pair(int civ_id, int other_id) {
    ...
    if (vassal_overlord(civ_id) >= 0) return other_id == vassal_overlord(civ_id) ? DIP_DISPLAY_VASSAL : DIP_DISPLAY_OTHER;
    if (vassal_is_direct(civ_id, other_id) || vassal_is_direct(other_id, civ_id)) return DIP_DISPLAY_VASSAL;
...
static int sovereign_id(int civ_id) {
    int over = vassal_overlord(civ_id);
...
static void draw_army_pool(HDC hdc, UiCursor *cursor, int civ_id) {
    char text[160], total[32], deployed[32], reserve[32], support[32], tribute[32];
...
static void sort_ids_by_army_desc(int *ids, int count) {
    ...
    for (i = 1; i < count; i++) {
        int key = ids[i], j = i - 1;
        while (j >= 0 && (war_current_soldiers_for_civ(ids[j]) < war_current_soldiers_for_civ(key) ||
               (war_current_soldiers_for_civ(ids[j]) == war_current_soldiers_for_civ(key) && ids[j] > key))) {
            ids[j + 1] = ids[j];
            j--;
        }
        ids[j + 1] = key;
...
static int status_order_for_vassal_other(DiplomacyStatus status) {
...
static DiplomacyEntry make_entry(int civ_id, int other_id, DiplomacyDisplayGroup group, int selected_is_vassal) {
    ...
    rel = diplomacy_relation(sort_a, sort_b);
    if (group == DIP_DISPLAY_PEACE_TENSE) {
        ...
        e.primary = vassal_is_direct(civ_id, other_id) ? 0 : 1;
        e.secondary = -war_current_soldiers_for_civ(other_id);
    } else {
...
static int collect_entries(int civ_id, DiplomacyDisplayGroup group, int selected_is_vassal,
    ...
    for (i = 0; i < civ_count; i++) {
        ...
        if (selected_is_vassal && group == DIP_DISPLAY_OTHER && i == vassal_overlord(civ_id)) continue;
...
int country_diplomacy_tab_height(int civ_id) {
    ...
    for (i = 0; i < civ_count; i++) {
        if (!direct_relation_visible(civ_id, i)) continue;
        if (display_group_for_pair(civ_id, i) != view && vassal_overlord(civ_id) < 0) continue;
        visible++;
        if (vassal_overlord(civ_id) < 0 && vassal_overlord(i) < 0) visible += vassal_direct_count(i);
    }
...
static int draw_diplomacy_group(HDC hdc, UiCursor *cursor, int civ_id,
    ...
    for (i = 0; i < count; i++) {
        int id = entries[i].id;
        if (selected_is_vassal && group == DIP_DISPLAY_OTHER) {
            draw_vassal_subrow(hdc, cursor, civ_id, id, vassal_overlord(civ_id));
        } else {
            draw_relation(hdc, cursor, civ_id, id);
            if (!selected_is_vassal && vassal_overlord(id) < 0) {
                int children[MAX_CIVS];
                int child_count = vassal_collect_direct(id, children, MAX_CIVS);
                int c;
