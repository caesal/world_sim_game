#include "load_progress_overlay.h"
...
void draw_load_progress_overlay(HDC hdc, RECT client) {
    ...
    stage_name = ui_language == UI_LANG_ZH ? load_progress_stage_name_zh(progress.stage) :
                                             load_progress_stage_name_en(progress.stage);
    ...
    draw_text_rect(hdc, title, tr("Loading map", "正在读取地图"),
