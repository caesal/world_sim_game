...
static void draw_overview(HDC hdc, UiCursor *cursor, int civ_id) {
    ...
    RECT card = ui_take_rect(cursor, 98);
    RECT bar = {card.left + 10, card.top + 36, card.right - 10, card.top + 50};
    RECT recovery_bar = {card.left + 10, card.top + 74, card.right - 10, card.top + 82};
    ...
    snprintf(text, sizeof(text), "%s +%s/%s   %s -%s/%s   %s %s/%s",
             ...
             tr("Recovery", "恢复"), b, tr("mo", "月"),
             tr("Actual change", "实际变化"), c, tr("mo", "月"));
    draw_text_rect(hdc, (RECT){card.left + 10, card.top + 54, card.right - 10, card.top + 72},
...
#undef REC_SEG
    ...
    draw_text_rect(hdc, (RECT){card.left + 10, card.top + 84, card.right - 10, card.bottom - 4},
...
static void draw_pressure_card(HDC hdc, RECT card, const PressureCard *src) {
    ...
    char eta[48];
    RECT stripe = card;
    RECT bar = {card.left + 10, card.top + 37, card.right - 10, card.top + 43};
    fill_rect(hdc, card, ui_theme_color(UI_COLOR_PANEL_SOFT));
    stripe.right = stripe.left + 4;
    fill_rect(hdc, stripe, src->color);
    draw_icon(hdc, src->icon, (RECT){card.left + 9, card.top + 6, card.left + 25, card.top + 22}, src->color);
    draw_text_rect(hdc, (RECT){card.left + 30, card.top + 4, card.right - 8, card.top + 22},
    ...
    draw_text_rect(hdc, (RECT){card.left + 30, card.top + 20, card.right - 8, card.top + 36},
                   text, ui_theme_color(UI_COLOR_TEXT_MUTED), DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
    ui_progress_bar(hdc, bar, src->value, 100, src->color);
    format_disorder_months(eta, sizeof(eta), src->eta_months);
    if (src->structural) snprintf(text, sizeof(text), "%s", tr("Hard floor", "硬下限"));
    ...
    draw_text_rect(hdc, (RECT){card.left + 10, card.top + 43, card.right - 10, card.top + 60},
                   text, ui_theme_color(UI_COLOR_TEXT), DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
    if (src->eta_months < 0) snprintf(text, sizeof(text), "%s", src->status);
    else snprintf(text, sizeof(text), "%s %s", tr("Clear", "清零"), eta);
    draw_text_rect(hdc, (RECT){card.left + 10, card.top + 60, card.right - 10, card.bottom - 5},
                   text, ui_theme_color(UI_COLOR_TEXT_MUTED), DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
}
...
static void draw_pressure_cards(HDC hdc, UiCursor *cursor, int civ_id) {
    ...
    PressureCard cards[6];
    int cols = cursor->width >= 340 ? 3 : 2;
    int gap = 6;
    int h = 80;
    ...
    cards[2] = (PressureCard){tr("War fatigue", "战争疲劳"), ICON_BATTLE, CIV(civ_id)->disorder_wartime,
                              ...
                              war_active ? -1 : disorder_ui_pressure_eta_months(CIV(civ_id)->disorder_wartime,
                                                                                 max(1, CIV(civ_id)->disorder_last_wartime_decay_x10 / 10)),
                              war_active ? tr("At war", "战争中") : tr("Monthly recovery", "逐月恢复"),
                              RGB(188, 86, 66), 0};
    cards[3] = (PressureCard){tr("War losses / unrest", "战损/内乱"), ICON_ATTACK, CIV(civ_id)->disorder_stability,
                              CIV(civ_id)->disorder_stability * 10 / 28, war_decay,
                              disorder_ui_pressure_eta_months(CIV(civ_id)->disorder_stability, war_decay),
                              war_active ? tr("Slow recovery during war", "战争中缓慢恢复") : tr("Post-war recovery", "战后恢复"),
                              RGB(188, 74, 66), 0};
    cards[4] = (PressureCard){tr("Migration / expansion", "迁徙/扩张"), ICON_MIGRATION, CIV(civ_id)->disorder_migration,
    ...
    cards[5] = (PressureCard){tr("Vassal governance", "附庸治理"), ICON_GOVERNANCE,
    ...
    cursor->y += ((6 + cols - 1) / cols) * (h + gap) + 2;
}
...
static void draw_plague_status(HDC hdc, UiCursor *cursor, int civ_id) {
    ...
    card = ui_take_rect(cursor, active > 0 ? 54 : 46);
    fill_rect(hdc, card, ui_theme_color(UI_COLOR_PANEL_SOFT));
    if (active > 0) {
...
static void format_effect_detail(char *out, size_t out_size, int tech_percent, int disorder_percent) {
    int tech_delta = tech_percent - 100;
    int disorder_drop = max(0, 100 - disorder_percent);
    snprintf(out, out_size, "%s 100%% / %s %+d%% / %s -%d%%",
             tr("Base", "基础"), tr("Tech", "科技"), tech_delta,
             tr("Disorder", "混乱"), disorder_drop);
}

static void draw_effect_bar(HDC hdc, UiCursor *cursor, IconId icon, const char *label,
                            int final_percent, int tech_percent, int disorder_percent,
                            COLORREF color) {
    char text[128];
    RECT row = ui_take_rect(cursor, 40);
    RECT icon_rect = {row.left, row.top + 5, row.left + 20, row.top + 25};
    RECT label_rect = {row.left + 26, row.top, row.right - 54, row.top + 20};
    RECT value_rect = {row.right - 52, row.top, row.right, row.top + 20};
    RECT bar = {row.left + 26, row.top + 21, row.right, row.top + 30};
    RECT detail_rect = {row.left + 26, row.top + 29, row.right, row.bottom};
    draw_icon(hdc, icon, icon_rect, color);
    draw_text_rect(hdc, label_rect, label, ui_theme_color(UI_COLOR_TEXT),
                   DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
    snprintf(text, sizeof(text), "%d%%", final_percent);
    ...
    format_effect_detail(text, sizeof(text), tech_percent, disorder_percent);
    draw_text_rect(hdc, detail_rect, text, ui_theme_color(UI_COLOR_TEXT_DIM),
                   DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
}
...
static void draw_effects(HDC hdc, UiCursor *cursor, int civ_id) {
    ...
    draw_effect_bar(hdc, cursor, ICON_PRODUCTION, tr("Resource output", "资源产出"),
                    tech_resource * resource_disorder / 100, tech_resource, resource_disorder, RGB(204, 154, 68));
    draw_effect_bar(hdc, cursor, ICON_POPULATION, tr("Population growth", "人口增长"),
                    population_disorder, 100, population_disorder, RGB(84, 164, 184));
    draw_effect_bar(hdc, cursor, ICON_INNOVATION, tr("Tech progress", "科技推进"),
...
static const char *collapse_risk_text(int civ_id) {
    ...
    else if (grace > 0) snprintf(text, sizeof(text), "%s", tr("No collapse risk", "无崩溃风险"));
    else if (chance <= 0) snprintf(text, sizeof(text), "%s", tr("No collapse risk", "无崩溃风险"));
    else snprintf(text, sizeof(text), "%s %d%%", tr("Collapse risk", "崩溃风险"), chance);
    return text;
}

static const char *single_collapse_expected_text(int result) {
    switch ((CollapseSingleProvinceResult)result) {
        case COLLAPSE_SINGLE_ANNEX_OVERLORD: return tr("Absorbed by overlord", "被宗主国接管");
        case COLLAPSE_SINGLE_ANNEX_WAR: return tr("Annexed by war opponent", "被交战国吞并");
        case COLLAPSE_SINGLE_ANNEX_NEIGHBOR: return tr("Annexed by neighbor", "被邻国吞并");
        case COLLAPSE_SINGLE_UNCLAIMED: return tr("Becomes unclaimed", "变成无主地");
        default: return "";
    }
}
...
static void draw_collapse_risk(HDC hdc, UiCursor *cursor, int civ_id) {
    ...
    int grace = CIV(civ_id)->collapse_grace_months;
    int chance = collapse_ui_chance_for_disorder(disorder);
    int single_result = CIV(civ_id)->decision.collapse_single_result;
    int takeover = CIV(civ_id)->decision.collapse_single_candidate;
    ...
    if (single_result != COLLAPSE_SINGLE_NONE) {
        snprintf(text, sizeof(text), "%s", tr("Single-province state cannot split.", "单省国家无法分裂。"));
    } else if (disorder < 75) snprintf(text, sizeof(text), "%s", tr("Disorder below 75", "混乱低于 75"));
    else if (grace > 0) snprintf(text, sizeof(text), "%s", tr("Grace active", "保护期"));
    else if (disorder >= 100) snprintf(text, sizeof(text), "%s", tr("Immediate check", "立即判定"));
    else snprintf(text, sizeof(text), "%s", tr("25-year check", "每 25 年判定"));
    draw_text_rect(hdc, (RECT){card.left + 12, card.top + 32, card.right - 12, card.top + 54},
                   text, ui_theme_color(UI_COLOR_TEXT_MUTED), DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
    if (single_result != COLLAPSE_SINGLE_NONE) {
        snprintf(text, sizeof(text), "%s: %s", tr("Takeover", "接管候选"),
                 takeover >= 0 ? snapshot_ui_civ_name(takeover) : tr("None", "无"));
    } else if (grace > 0) format_disorder_months(span, sizeof(span), grace);
    else if (chance > 0) {
    ...
    draw_text_rect(hdc, (RECT){card.left + 12, card.top + 56, card.right - 12, card.top + 76},
                   text, ui_theme_color(UI_COLOR_TEXT), DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
    if (single_result != COLLAPSE_SINGLE_NONE) {
        snprintf(text, sizeof(text), "%s: %s", tr("Expected", "预计结果"),
                 single_collapse_expected_text(single_result));
        draw_text_rect(hdc, (RECT){card.left + 12, card.top + 74, card.right - 12, card.bottom - 8},
