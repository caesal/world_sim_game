...
static int build_owner_surface_fill(MapLayerCache *cache,
    ...
    if (!map_ownership_surface_snapshot_view(snapshot, &surface)) return 0;
    civ_total = clamp(snapshot ? snapshot->civ_count : 0, 0, MAX_CIVS);
    memset(active, 0, sizeof(active));
    for (x = 0; x < civ_total; x++) {
        int ok = map_display_policy_snapshot_owner_fill(
            snapshot, x, display_mode, &fills[x]);
...
void render_static_map_cache_build_fill_pixels(MapLayerCache *cache,
    ...
    if (build_owner_surface_fill(cache, snapshot)) return;
    width = snapshot->map_w;
    height = snapshot->map_h;
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            const SnapshotTile *tile = &snapshot->tiles[y * snapshot->map_w + x];
            MapDisplayFillPolicy fill;
            if (!map_display_policy_snapshot_fill(
                    snapshot, tile, display_mode, &fill)) continue;
