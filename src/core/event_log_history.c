...
static int civ_event_ids[MAX_CIVS][EVENT_LOG_CIV_HISTORY_COUNT];
...
static void store_one(int civ_id, int civ_uid, const EventLogEntry *entry, int event_id) {
    int pos;
    if (!entry || event_id <= 0 || civ_id < 0 || civ_id >= MAX_CIVS || civ_uid <= 0) return;
    if (!event_log_entry_involves_civ_uid(entry, civ_id, civ_uid)) return;
    pos = civ_event_next[civ_id];
    civ_event_ids[civ_id][pos] = event_id;
...
void event_log_history_store_related_id(const EventLogEntry *entry, int event_id) {
    if (!entry || event_id <= 0 || !event_type_is_country_scoped_history(entry->type)) return;
    store_one(entry->civ_id, entry->civ_uid, entry, event_id);
    if (entry->target_id != entry->civ_id || entry->target_uid != entry->civ_uid) {
        store_one(entry->target_id, entry->target_uid, entry, event_id);
    }
    if (event_param_a_is_civ_history(entry->type) &&
        ...
        store_one(entry->param_a, entry->param_a_uid, entry, event_id);
    }
}
...
void event_log_history_clear(void) {
    memset(civ_event_ids, 0, sizeof(civ_event_ids));
...
int event_log_recent_count_for_civ_uid(int civ_id, int civ_uid) {
    ...
    for (i = 0; i < civ_event_count[civ_id]; i++) {
        int id = civ_event_ids[civ_id][i];
        if (event_log_store_get_by_id(id, &entry) &&
            event_log_entry_involves_civ_uid(&entry, civ_id, civ_uid)) count++;
    }
    return count;
}
...
int event_log_recent_for_civ_uid(int civ_id, int civ_uid, int index, EventLogEntry *out) {
    ...
    for (checked = 0; checked < civ_event_count[civ_id]; checked++) {
        ...
        if (!event_log_store_get_by_id(civ_event_ids[civ_id][pos], &entry)) continue;
        if (!event_log_entry_involves_civ_uid(&entry, civ_id, civ_uid)) continue;
        if (seen++ == index) {
            *out = entry;
            return 1;
        }
