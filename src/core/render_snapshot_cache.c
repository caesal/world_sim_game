...
void render_snapshot_cache_reset(void) {
    ...
    render_snapshot_plague_cache_reset();
    city_cursor = pair_cursor = 0;
    lane_cache_key = 0;
    lane_cache_valid = lane_cache_count = 0;
...
void render_snapshot_cache_begin_snapshot_copy(void) {
    copy_city_cached = copy_city_stale = copy_city_fallback = 0;
    copy_pair_cached = copy_pair_stale = copy_pair_fallback = 0;
    render_snapshot_plague_cache_begin_snapshot_copy();
    last_lane_source = "none";
}
...
void render_snapshot_cache_update_budgeted(int city_budget, int pair_budget,
    ...
    if (world_generated && refresh_plague_cache &&
...
void render_snapshot_cache_update_all(void) {
    ...
    if (world_generated) {
        refresh_lanes(lane_key);
        updated++;
        render_snapshot_plague_cache_refresh(plague_key);
        updated++;
    } else {
        lane_cache_valid = 0;
        lane_cache_count = 0;
        render_snapshot_plague_cache_invalidate();
    }
    city_cursor = 0;
    pair_cursor = 0;
...
int render_snapshot_cache_dirty_count(void) {
    ...
    if (world_generated && render_snapshot_plague_cache_is_dirty(plague_key)) count++;
    return count;
}
