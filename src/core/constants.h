#ifndef WORLD_SIM_CONSTANTS_H
#define WORLD_SIM_CONSTANTS_H

#define MAX_MAP_W 1152
#define MAX_MAP_H 800
#define MAP_SIZE_SMALL 0
#define MAP_SIZE_MEDIUM 1
#define MAP_SIZE_LARGE 2
#define MAP_SIZE_EXTREME 3
#define MAP_SIZE_COUNT 4
#define SPEED_COUNT 5
#define MAP_DISPLAY_MODE_COUNT 6
#define ALLIANCE_DETAIL_TAB_STORAGE_COUNT 5
#define CIV_COLOR_PALETTE_COUNT 16
#define DEFAULT_MAP_W 720
#define DEFAULT_MAP_H 500
extern int map_w;
extern int map_h;
#define MAP_W map_w
#define MAP_H map_h
#define MAX_CIVS 200
#define MAX_NATURAL_REGIONS 1536
#define MAX_CITIES MAX_NATURAL_REGIONS
#define MAX_REGIONS MAX_NATURAL_REGIONS
#define MAX_REGION_NEIGHBORS 16
#define REGION_DIR_COUNT 8
#define NAME_LEN 64
#define WINDOW_W 1920
#define WINDOW_H 1080
#define TOP_BAR_H 62
#define DEFAULT_SIDE_PANEL_W 520
#define MIN_SIDE_PANEL_W 500
#define MAX_SIDE_PANEL_W 720
#define SIDE_PANEL_COLLAPSED_W 34
#define BOTTOM_BAR_H 44
#define TIMER_ID 1
#define MAP_PREVIEW_TIMER_ID 2
#define WHEEL_INPUT_TIMER_ID 3
#define FRAME_TIMER_MS 16
#define FORM_X_PAD 18
#define CITY_MIN_DISTANCE 18
#define MAX_POPULATION 2000000000
#define MAX_RIVER_PATHS 96
#define MAX_RIVER_POINTS 280
#define MAX_MARITIME_ROUTES 160
#define MAX_MARITIME_ROUTE_POINTS 96
#define POP_COHORT_COUNT 8
#define EVENT_LOG_COUNT 256
#define EVENT_LOG_LEN 160
#define EVENT_LOG_CIV_HISTORY_COUNT 50

#endif
